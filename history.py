...
class FractalApp(QObject):
    ...
    def update_plot(self):
        self._root_widget._status.setText("Calculating set...")  # = "calculating..."
        #self._root_widget._status.show()
        with SharedMemoryManager() as smm:
            ...
            i = int(self._root_widget.processes.text())
            tasks, data = self._fractal1.generate_tasks(smm, i)
            ...
            #for self.task in self.tasks: task()
            #self.image_matrix = self.fractal1.data_to_image_matrix(self.data)
            #self._axes.imshow(image.imread(self.image))
            #sg = "These are definitely some words."
            #print(slist := smm.ShareableList(msg))

            #proc = Process(target=self.fractal1.generate_tasks(), args=(slist,20))
            #proc.start()
            #time.sleep(.05)
        #self._root_widget.axes = image_matrix
        if self._image == None:
        ...
        #self._root_widget.axes.imshow(image)
        #imshow(image)
    #connects to signal for entering after a textbox
